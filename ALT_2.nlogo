; both the foxes and the rabbits have energy that will:
;     - decrease as they move and;
;     - replenish once they feed (fox -> rabbit; rabbit -> carrot)
turtles-own [ energy ]

; the patches will serve as the carrots which will be the food for the rabbits
patches-own [ time-to-grow ]

; Sheep and wolves are both breeds of turtles
breed [ rabbits rabbit ]  ; sheep is its own plural, so we use "a-sheep" as the singular
breed [ foxes fox ]
...
to setup
  ...
  ;setup random patches to carrot / dirt (brown)
  ask patches [
    set pcolor one-of [ orange brown ]
    ifelse pcolor = orange
      [ set time-to-grow 20 ]
    [ set time-to-grow random 20 ]
  ]

  ;create rabbits based on value set on slider
  create-rabbits initial-rabbits
  [
    set shape "rabbit"
    set color white
    set energy 100 ;initial energy
    setxy random-xcor random-ycor ;random place
  ]

  ;create foxes based on value set on slider
  ...
  reset-ticks ;reset ticks for plot
end
...
;consume carrots (orange patches)
to eat-carrot
  if pcolor = orange [
    set pcolor brown
    set energy energy + carrot-energy
  ]
end

;consume rabbits
to eat-rabbit
  let feeders foxes with [any? rabbits-here]
  if any? feeders
  [
...
;add turtles of the same breed depending on the probability set on the slider
to reproduce-rabbit
